...
bool waketime_compare (struct list_elem* elem1, struct list_elem* elem2, void* aux);
...
/* Create child thread */
struct child *child_create (struct thread *thread_parent);
/* Return child with tid if it exist in the list else return null */
struct child *search_child (tid_t tid, struct list* children_list);
...
struct file_descriptor *search_fd (int fd);
