...
load (const char *file_name, void (**eip) (void), void **esp) 
{
  ...
  *eip = (void (*) (void)) ehdr.e_entry;
  ...
  /* We arrive here irrespective of load being successful or not. */
  if (success)
  {
...
install_page (void *upage, void *kpage, bool writable)
{
  ...
  /* Verification whether there is a page already or not at that virtual
...
bool process_add_mmap (struct sp_entry *spe) {
  ...
  if (!mmf) return false;
  mmf->spe = spe;
  mmf->mapid = thread_current()->mapid;
  ...
  while(e!=list_end(&t->mmap_list)){
    ...
    if(mmf->mapid == map || map == -1){
      ...
      if(mmf->spe->loaded){
        ...
        frame_free(pagedir_get_page(t->pagedir,mmf->spe->uv_add));
